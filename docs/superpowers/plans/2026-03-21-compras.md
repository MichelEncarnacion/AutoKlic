# Compras Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a Compras section where the dealership can register car acquisitions with purchase details, seller info, payment method, documentation status, and itemized additional expenses.

**Architecture:** Three tasks — (1) manual DB migration in Supabase, (2) create `Compras.jsx` as the full-featured page, (3) wire the route and sidebar link. `Compras.jsx` holds all state locally: compras list, lazily-loaded gastos per row (keyed by compra_id), and form state for two modals plus per-row inline expense forms.

**Tech Stack:** React 19, Vite 7, Supabase JS v2, Tailwind CSS 3, Heroicons 24/outline, date-fns, react-hot-toast.
...
|------|--------|----------------|
| `src/pages/admin/Compras.jsx` | **Create** | Full purchases page — table, expandable rows, modals |
| `src/App.jsx` | **Modify** | Import Compras; add child route `path="compras"` |
| `src/pages/admin/AdminLayout.jsx` | **Modify** | Add `ShoppingCartIcon`; add Compras to `navItems` |
...
- [ ] **Step 1: Create the `compras` table**
...
- [ ] **Step 2: Create the `gastos_compra` table**
...
alter table gastos_compra enable row level security;
```

- [ ] **Step 4: Create RLS policies for `compras`**

```sql
-- SELECT: admin sees all; seller sees only own
...
-- INSERT: admin and seller; created_by must be caller's own id
...
- [ ] **Step 5: Create RLS policies for `gastos_compra`**
...
In the Supabase Table Editor, confirm both tables exist with the correct columns.

---

## Task 2: Create `src/pages/admin/Compras.jsx`

**Files:**
- Create: `src/pages/admin/Compras.jsx`

Context:
- `src/lib/supabase.js` — `import { supabase } from '../../lib/supabase'`
- `src/context/AuthContext.jsx` — `import { useAuth } from '../../context/AuthContext'` → `{ profile }` where `profile.role` is 'admin' | 'seller' | 'viewer'
- `react-hot-toast` is bootstrapped at app root — just import `toast`; no `<Toaster />` needed
- `date-fns` v3 is installed — use `format` and `parseISO`
- Heroicons 24/outline — `ChevronDownIcon`, `ChevronUpIcon`, `TrashIcon`

- [ ] **Step 1: Create the file** with the complete implementation below:
...
- "Cannot find module '../../lib/supabase'" → check relative path (file is in `src/pages/admin/`)
- "ChevronDownIcon is not exported" → verify the heroicons import

- [ ] **Step 3: Commit**

```bash
git add src/pages/admin/Compras.jsx
git commit -m "feat(compras): add car purchase registration page"
```
...
- [ ] **Step 1: Read `src/App.jsx`** to find the current admin imports and route block.

- [ ] **Step 2: Add the import** after the existing `import Rendimiento` line:

```js
import Compras from './pages/admin/Compras'
```

- [ ] **Step 3: Add the child route** inside `<Route path="/admin">`, after `<Route path="rendimiento" ...>`:
...
- [ ] **Step 4: Read `src/pages/admin/AdminLayout.jsx`** to see the current Heroicons import and navItems.

- [ ] **Step 5: Add `ShoppingCartIcon`** to the existing `@heroicons/react/24/outline` import alongside the other icons.

- [ ] **Step 6: Add "Compras" to `navItems`** after the `rendimiento` entry (which currently is at index 4, before `usuarios` at index 5):
...
Run `npm run dev`:
- Log in as **admin**: sidebar shows "Compras" between "Rendimiento" and "Usuarios".
- Log in as **seller**: sidebar shows "Compras" (no "Usuarios").
- Navigate to `/admin/compras`: page loads with empty state and "Registrar compra" button.
- Click "Registrar compra": modal opens with all fields. Fill precio_compra + fecha_compra and save — row appears in table.
- Expand the row: "Sin gastos registrados" + inline form. Add a gasto — appears in list.
- Click doc badges: toggle between green ✓ and gray ✗.
- As admin, click delete: confirmation modal appears. Confirm — row removed.

- [ ] **Step 9: Commit**

```bash
git add src/App.jsx src/pages/admin/AdminLayout.jsx
git commit -m "feat(compras): add route and sidebar link"
```
